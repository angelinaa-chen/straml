...
let () =
  ...
  ignore
    (start_button#connect#clicked ~callback:(fun () ->
         ...
         let theme =
           match grid with
           | g when g == GridData.initial_grid -> "Fall Fun"
           | g when g == GridData.nice_fit -> "Well-Suited"
           | g when g == GridData.to_your_health -> "To Your Health"
           | g when g == GridData.extremely_online -> "Extremely Online"
           | _ -> "Invalid theme :("
         in
         ...
         let initial_state = Cs3110_fin.Logic.initialize_game grid in
