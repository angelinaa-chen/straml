...
let test_make_state _ =
  ...
  let test_print_letter _ =
    ...
    Format.pp_print_flush fmt (); (* Ensure all contents are flushed to the buffer *)
...
let test_process_input _ =
  ...
  let match_state = process_input initial_state "target" target_words match_counter hint_counter max_hints accepted_words in
  assert_equal match_state.found_words ["target"];
  assert_equal !match_counter 1;

  let hint_state = process_input match_state "hint" target_words match_counter hint_counter max_hints accepted_words in
  assert_equal hint_state.found_words ["target"];
  assert_equal !hint_counter 1;

  let invalid_state = process_input hint_state "invalid" target_words match_counter hint_counter max_hints accepted_words in
